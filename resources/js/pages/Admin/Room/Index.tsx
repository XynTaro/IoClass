...
import { Archive, DoorOpen, RotateCcw, Trash2 } from 'lucide-react';
import { useEffect, useMemo, useState } from 'react';
import { IoMdAdd, IoMdCreate, IoMdSearch } from 'react-icons/io';
import { route } from 'ziggy-js';
import ConfirmationModal from '@/components/ConfirmationModal';
import { DataTable } from '@/components/DataTable';
import RoomModal, { type BuildingOption, type Room } from '@/components/RoomModal';
...
export default function Index() {
    const { rooms, archived = false, buildings = [] } = usePage<{
        rooms: any;
        archived: boolean;
    ...
    const [itemToArchive, setItemToArchive] = useState<Room | null>(null);
    const [isArchiving, setIsArchiving] = useState(false);
    const [restoreConfirmOpen, setRestoreConfirmOpen] = useState(false);
    const [itemToRestore, setItemToRestore] = useState<Room | null>(null);
    ...
    const filteredRooms = useMemo(() => {
        ...
        return roomList.filter((r) =>
            [r.room_no ?? '', r.building?.building_name ?? ''].join(' ').toLowerCase().includes(q),
        );
    }, [search, roomList]);
    ...
    const handleConfirmArchive = () => {
        ...
        setIsArchiving(true);
        router.delete(route('admin.room.destroy', { room: itemToArchive.room_id }), {
            onSuccess: () => {
    ...
    const handleConfirmRestore = () => {
        ...
        setIsRestoring(true);
        router.post(
            route('admin.room.restore', { id: itemToRestore.room_id }),
            {},
            {
                onSuccess: () => {
    ...
    const columns: ColumnDef<Room>[] = [
        ...
        },
        {
            ...
            header: 'Building',
            cell: ({ row }) => row.original.building?.building_name ?? '—',
        },
        {
            id: 'actions',
            header: 'Actions',
    ...
    return (
        <AdminLayout>
            <Head title={archived ? 'Archived Rooms' : 'Rooms'} />
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="space-y-1">
                            <h1 className="text-2xl font-bold tracking-tight md:text-3xl">
                                {archived ? 'Archived Rooms' : 'Rooms Management'}
                            </h1>
                            <p className="text-sm text-muted-foreground">
                                {archived
                                    ? 'View and manage soft-deleted classroom records.'
                                    : 'Manage campus rooms, capacities, and building assignments.'}
                            </p>
                        </div>
                        <div className="flex flex-wrap items-center gap-2">
                            <span className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-background/80 px-3 py-1.5 text-xs font-semibold text-muted-foreground shadow-xs backdrop-blur-sm">
                                <DoorOpen className="size-3.5 text-emerald-600 dark:text-emerald-400" />
                                {roomList.length} {archived ? 'Archived' : 'Rooms'}
                            </span>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div className="relative w-full sm:w-72">
                                ...
                                <Input
                                    ...
                                    placeholder="Search by room no. or building..."
                                ...
                                <Link
                                    href={
                                        archived
                                            ...
                                            : route('admin.room.index', { archived: 1 })
                                    }
            ...
            <ConfirmationModal
                ...
                description={
                    itemToArchive
                        ? `Are you sure you want to archive room ${itemToArchive.room_no}?`
                        : ''
                }
                type="archive"
            ...
            <ConfirmationModal
                ...
                description={
                    itemToRestore
                        ? `Are you sure you want to restore room ${itemToRestore.room_no}?`
                        : ''
                }
                type="restore"
